...
import com.baiyun2.httputils.SchoolLifeHttpUtils.onGetGuideListener;
...
import com.baiyun2.vo.parcelable.LifeGuidePar;
...
public class RecruitHttpUtils  extends HttpUtils{
	...
	public interface OnGetRecruitListListener{
		public void onRecruitList(List<RecruitPar> recruitPars);
	}
...
//				System.out.println(responseInfo.result);
	...
	public void getRecruitList(final OnGetRecruitListListener getRecruitListListener){
		String url = HttpURL.RECRUIT_GET_INFO;
		send(HttpMethod.GET, url, new RequestCallBack<String>() {
			
			@Override
			public void onSuccess(ResponseInfo<String> responseInfo) {
				List<RecruitPar> recruitPars = null;
				try {
					JsonParser parser = new JsonParser();
					JsonObject jsonObject = parser.parse(responseInfo.result).getAsJsonObject();
					...
					if (recodeEle.isJsonPrimitive()) {
						...
						if (recode.equalsIgnoreCase(HttpRecode.GET_SUCCESS)) {
							...
							if (dataEle.isJsonArray()) {
								JsonArray jsonArray = dataEle.getAsJsonArray();
								java.lang.reflect.Type type = new TypeToken<List<RecruitPar>>() {}.getType();
				...
				getRecruitListListener.onRecruitList(recruitPars);
			}

			@Override
			public void onFailure(HttpException error, String msg) {
				getRecruitListListener.onRecruitList(null);
				Toast.makeText(context, "数据请求失败", Toast.LENGTH_SHORT).show();
			}
